...
`main.cpp` builds an `Experiment` (often via `JsonExperiment`), which
constructs one `Driver` per policy from `policies/` or `mcts/`, each wrapping
its own `Simulator` fed by `SimJob`s from `io/swf_reader`. Each `Driver` runs
its timeline to completion in `SimMode::Real` and the `Experiment` aggregates
metrics across all of them.

## Diagram (draw.io sketch)

A simple box-and-arrow diagram to capture the structure above:
...
│   │ (policy: FIFO)│   │ (policy: WFP) │   │ (policy: MCTS)│         │
│   │               │   │               │   │               │         │
│   │ ┌───────────┐ │   │ ┌───────────┐ │   │ ┌───────────┐ │         │
│   │ │ Simulator │ │   │ │ Simulator │ │   │ │ Simulator │ │         │
│   │ │(SimMode:  │ │   │ │(SimMode:  │ │   │ │(SimMode:  │ │         │
│   │ │  Real)    │ │   │ │  Real)    │ │   │ │  Real)    │ │         │
│   │ └───────────┘ │   │ └───────────┘ │   │ └──────┬────┘ │         │
│   └───────────────┘   └───────────────┘   └────────┼──────┘         │
│                                                      │ snapshot       │
...
└─────────────────────────────────────────────────────────────────────┘
        ...
        ▲                          ▲                         ▲
        │ SimJob / NodeInfo        │ Decision / State         │
        │                          │                          │
┌───────┴───────┐         ┌────────┴────────┐        ┌────────┴────────┐
│  io/           │         │  core/          │        │  policies/       │
│  swf_reader,   │         │  Job, NodeInfo, │        │  FIFO, SJF, WFP, │
│  maintenance_  │         │  State,         │        │  SCF, F1-F4, ... │
│  reader        │         │  Decision,      │        │  (SchedulerPolicy│
│                │         │  SchedulerPolicy│        │   impls)         │
└────────────────┘         └─────────────────┘        └──────────────────┘
```

Suggested draw.io layout:
1. One big container box for `Experiment`, containing a row of `Driver` boxes.
2. Each `Driver` box nests a `Simulator` box (label it with its `SimMode`).
3. One `Driver` (the MCTS one) has an arrow labeled "snapshot" pointing down/out
   to a separate `Mcts search tree` box, itself containing several small nested
   `Simulator (Prediction)` boxes to suggest the cloned tree nodes.
4. Below, three peer boxes for `io`, `core`, `policies`, with arrows up into
   the `Experiment`/`Driver`/`Simulator` boxes showing what feeds what
   (`io` → job/node data in; `core` → shared types used everywhere; `policies`
   → plugged into `Driver` as the `SchedulerPolicy`).
