#!/usr/bin/env python3
"""Plotting script for cleaned Theta trace experiments.

Theta job-size tiers (consistent colors throughout):
  T1 — 128–255  nodes  (steelblue)   max walltime  3 h
  T2 — 256–383  nodes  (darkorange)  max walltime  6 h
  T3 — 384–639  nodes  (seagreen)    max walltime  9 h
  T4 — 640–801  nodes  (crimson)     max walltime 12 h
  T5 — 802–4096 nodes  (mediumpurple) max walltime 24 h
...
Per-driver output files  (<plots_dir>/<driver_tag>/):
  ...
  cdf_wait_T1.png            — wait-time CDF for T1 tier, P25/P50/P75/P99 marked
  cdf_wait_T2.png            — wait-time CDF for T2 tier
  cdf_wait_T3.png            — wait-time CDF for T3 tier
  cdf_wait_T4.png            — wait-time CDF for T4 tier
  cdf_wait_T5.png            — wait-time CDF for T5 tier
  cdf_bsld_T1.png            — BSLD CDF for T1 tier
  ...
...
Global output files  (<plots_dir>/):
  cdf_wait_cdf.png           — wait-time CDF, all tiers, all drivers
  cdf_bsld.png               — BSLD CDF, all tiers, all drivers
  ...
  cdf_wait_T{1-5}.png        — wait-time CDF per tier, all drivers
  cdf_bsld_T{1-5}.png        — BSLD CDF per tier, all drivers
  table_T{1-5}_wait.csv      — wait-time stats per tier
  table_T{1-5}_bsld.csv      — BSLD stats per tier
  scatter_wait_vs_size_T{1-5}.png — per-driver scatter, one file per tier
...
RLScheduler support:
    ...
    Example driver entry in the experiment JSON:
        {"type": "rlscheduler", "tag": "RLScheduler",
         "csv_path": "data/theta21cln_rlscheduler.csv"}

Usage:
    python3 scripts/thetacln_plot.py experiments/exp1a.json
    python3 scripts/thetacln_plot.py experiments/exp7a.json --plots-dir results/custom_plots
"""
...
# ── Theta job-size tiers ──────────────────────────────────────────────────────
GROUP_LABELS  = ['T1', 'T2', 'T3', 'T4', 'T5']
GROUP_DISPLAY = {
    'T1': 'T1 (128–255)',
    'T2': 'T2 (256–383)',
    'T3': 'T3 (384–639)',
...
# Tier x-ranges used for scatter zone bands and percentile line spans
_ZONE_X = {
    'T1': (128,  255),
    'T2': (256,  383),
    'T3': (384,  639),
...
# Darker shades for percentile lines
_ZONE_DARK = {
    'T1': '#1a527a',
    'T2': '#994d00',
    'T3': '#1a6640',
...
# Zoomed x-limits for per-tier global scatter subplots
_GROUP_XLIM = {
    'T1': (115,  270),
    'T2': (240,  410),
    'T3': (370,  660),
...
# Argonne / Chicago local time — matches Theta submit timestamps
CST = timezone(timedelta(hours=-6))
ANALYSIS_YEAR = 2021

# Theta system size (total nodes)
S_SYS = 4096
...
def assign_group(num_procs):
    """Return 'T1'–'T5' based on Theta tier node ranges, or None."""
    if 128  <= num_procs <= 255:  return 'T1'
    if 256  <= num_procs <= 383:  return 'T2'
    if 384  <= num_procs <= 639:  return 'T3'
    if 640  <= num_procs <= 801:  return 'T4'
    if 802  <= num_procs <= 4096: return 'T5'
    return None
...
def plot_per_driver_group_bsld_cdfs(tag, job_ids, wait_map, walltimes_map,
    ...
    for jid in job_ids:
        ...
        group_bsld[g].append((wait_map[jid] + walltime) / walltime)
    for g in GROUP_LABELS:
        fig, ax = plt.subplots(figsize=(FIG_W, FIG_H))
...
# ── Per-driver: wait vs size scatter ─────────────────────────────────────────

def _draw_tier_zones(ax, fixed_ymax):
    """Draw zone backgrounds and tier labels on a scatter axis."""
    label_y = fixed_ymax * 0.9
    for g in GROUP_LABELS:
        x_lo, x_hi = _ZONE_X[g]
        ax.axvspan(x_lo, x_hi, color=GROUP_COLORS[g], alpha=0.12, zorder=1)
        mid = np.exp((np.log(x_lo) + np.log(x_hi)) / 2)
        ax.text(mid, label_y, g, ha='center', fontweight='bold',
                alpha=0.55, fontsize=11)


def _draw_reward_lines(ax, driver_cfg, tag, x_lo=128, x_hi=4096):
    """Overlay the reward=0.5 iso-line for queuesnapshot drivers.

    QueueSnapshot: per-job reward = A_eff/(wait + A_eff), A_eff = A*(B/S)^n.
    The reward=0.5 iso-curve is wait_h = (A/3600) * (B/S)^n.
    """
    reward_type = driver_cfg.get('reward_type', '')
    if reward_type not in ('queuesnapshot', 'harmonic_wait_sized'):
        return
    # Support both old (reward_param_a/n) and current (size_adj_A/n) key names
    ...
    if A is None:
        return
    B_vals = np.linspace(x_lo, x_hi, 500)
    w_half = (A / 3600.0) * (B_vals / S_SYS) ** n
    ax.plot(B_vals, w_half, color='black', linestyle='-',
...
def plot_per_driver_wait_vs_size_scatter(tag, job_ids, wait_map, procs_map,
    ...
    for jid in job_ids:
        ...
        if jid in backfill_ids:
            sizes_bf.append(procs_map[jid]); waits_bf.append(w_h)
        else:
            sizes_run.append(procs_map[jid]); waits_run.append(w_h)
    ...
    _draw_reward_lines(ax, driver_cfg, tag)
    _draw_tier_zones(ax, fixed_ymax)

    # Per-tier percentile lines
...
def plot_per_driver_bsld_vs_size_scatter(tag, job_ids, wait_map, procs_map,
    ...
    for jid in job_ids:
        ...
        if jid in backfill_ids:
            sizes_bf.append(procs_map[jid]); bsld_bf.append(bsld)
        else:
            sizes_run.append(procs_map[jid]); bsld_run.append(bsld)
    ...
    # Reward iso-line at reward=0.5 for the driver's reward function
    reward_type = driver_cfg.get('reward_type', '')
    A = driver_cfg.get('reward_param_a')
    n = driver_cfg.get('reward_param_n', 1.0)
    if A is not None:
        B_values = np.linspace(128, 4096, 500)
        if reward_type == 'harmonic_bsld_sized':
            ...
            # reward=wait/(wait+A*(S/B)^(1/n)); reward=0.5 when wait=A*(S/B)^(1/n)
            # Convert to BSLD using median walltime
...
def plot_per_driver_size_wait_kde(tag, job_ids, wait_map, procs_map,
                                   plots_dir, driver_cfg=None,
                                   class_ymax=None):
    """size_wait_kde.png — 2-D KDE of (job size [log], wait time) per tier."""
    try:
    ...
    _KDE_CMAPS = {'T1': 'plasma', 'T2': 'inferno', 'T3': 'magma',
                  'T4': 'viridis', 'T5': 'cividis'}
    _GC = '#333333'

    fig, axes = plt.subplots(1, 5, figsize=(FIG_W * 2.5, FIG_H + 1))
...
# ── Per-driver: normal-only and backfill-only scatter ────────────────────────
...
def _draw_violin_row(axes_by_group, walltimes_by_group):
    for g in GROUP_LABELS:
        ...
        ax.set_title(GROUP_DISPLAY[g], fontsize=FONT_SIZE - 3)
        ax.set_xticks([])
        ax.set_ylabel('Walltime (h)', fontsize=FONT_SIZE - 4)
...
def plot_per_driver_scatter_normal(tag, job_ids, wait_map, procs_map,
    ...
    for jid in job_ids:
        ...
        sizes.append(procs_map[jid]); waits.append(w_h)
        waits_by_group[g].append(w_h)
        walltimes_by_group[g].append(wt_h)

    fig = plt.figure(figsize=(FIG_W + 4, FIG_H + 3))
    gs  = fig.add_gridspec(2, 5, height_ratios=[1, 2.5], hspace=0.45, wspace=0.35)
...
def plot_per_driver_wait_vs_submit_time(tag, job_ids, wait_map, submit_map,
                                        procs_map, plots_dir):
    """scatter_wait_vs_submit_time.png — y: wait (h), x: submit datetime, color: tier."""
...
# ── Global: scatter wait vs size — per-driver subplots, one file per tier ────
...
# ── Per-driver: Wait KDE per tier (with reward threshold bands) ───────────────

def plot_per_driver_wait_kde_by_tier(tag, job_ids, wait_map, procs_map,
                                      plots_dir, driver_cfg=None):
    """wait_kde_by_tier.png — one subplot per tier, KDE of wait time 0-120 h.

    Vertical bands show the r=0.5 and r=0.99 reward thresholds across the
    tier's node-size range (harmonic_wait_sized drivers only).
    """
    try:
        from scipy.stats import gaussian_kde
    except ImportError:
    ...
    def _reward_thresh(r, B):
        """Wait threshold (h) for reward=r at node count B."""
    ...
    for idx, g in enumerate(GROUP_LABELS):
        ...
        # Reward threshold bands across the tier's size range
        if has_reward:
            x_lo, x_hi = _ZONE_X[g]
            for r_val, r_color, r_label in [
                ...
                t_lo = _reward_thresh(r_val, x_hi)  # smaller jobs → larger threshold
    ...
    fig.suptitle(f'Wait KDE by Tier — {tag}', fontsize=FONT_SIZE)
    fig.tight_layout()
...
def plot_global_wait_kde_by_group(driver_tags, all_wait_by_group, driver_colors,
                                   plots_dir):
    """wait_kde_{T1-T5}.png — 1-D KDE subplots per driver, shared x-axis 0-120 h."""
    try:
        from scipy.stats import gaussian_kde
    except ImportError:
...
def plot_global_overall_wait_cdfs(driver_tags, all_wait, driver_colors, plots_dir):
    ...
    lbls    += [f'P{p}' for p in PERCENTILES]
...
def plot_global_group_wait_cdfs(driver_tags, all_wait_by_group, driver_colors,
    ...
    for g in GROUP_LABELS:
        fig, ax = plt.subplots(figsize=(FIG_W, FIG_H))
        for tag in driver_tags:
            _draw_cdf_multi(ax, all_wait_by_group[tag].get(g, []),
                            driver_colors[tag], tag)
        apply_style(ax, xlabel="Wait Time (hours)", ylabel="CDF")
        ax.set_ylim(0, 1.05)
        ax.set_xscale('log')
        handles, lbls = ax.get_legend_handles_labels()
...
def plot_global_group_bsld_cdfs(driver_tags, all_bsld_by_group, driver_colors,
    ...
    for g in GROUP_LABELS:
        fig, ax = plt.subplots(figsize=(FIG_W, FIG_H))
        for tag in driver_tags:
            _draw_cdf_multi(ax, all_bsld_by_group[tag].get(g, []),
                            driver_colors[tag], tag)
        apply_style(ax, xlabel="Bounded Slowdown (Walltime)", ylabel="CDF")
        ax.set_ylim(0, 1.05)
        handles, lbls = ax.get_legend_handles_labels()
...
_TAIL_NODE_EDGES  = [128, 256, 384, 640, 802, 1024, 2048, 4097]
_TAIL_NODE_LABELS = ['T1\n128–255', 'T2\n256–383', 'T3\n384–639',
                     'T4\n640–801', 'T5a\n802–1023', 'T5b\n1024–2047',
                     'T5c\n2048–4096']
_TAIL_WT_EDGES    = [0, 1, 2, 3, 6, 12, 24.01]   # hours
_TAIL_WT_LABELS   = ['0–1h', '1–2h', '2–3h', '3–6h', '6–12h', '12–24h']
...
def main(argv=None):
    ...
    driver_tags = [d.get("tag", d["type"]) for d in config.get("drivers", [])
                   if d.get("plot", True)]
    tag_to_driver_cfg = {d.get("tag", d["type"]): d for d in config.get("drivers", [])
                         if d.get("plot", True)}
    if not driver_tags:
    ...
    for tag in driver_tags:
        ...
        if not end_f:
            print(f"[skip] {tag}: no completed jobs submitted within {ANALYSIS_YEAR}")
            continue
